...
export const CreateElection = () => {
  ...
  const [openDropdown, setOpenDropdown] = useState(null); // "pi_ci" key of currently open suggestion box

  useEffect(() => {
    // FIX: AdminHeader caches users under the key 'users', not 'allusers'.
    // Read the correct key, and if it's empty (cache hasn't run yet),
    // fetch live from the server as a fallback.
  ...
  const handleUserSearch = (pi, ci, query) => {
    ...
    setOpenDropdown(key);
    // Free-typed name until a suggestion is explicitly tapped
  ...
  // on how/when it was uploaded (array of {url}, array of plain strings, or
  // a single string on older records) — check all of them so we don't miss it.
  ...
  const selectSuggestion = (pi, ci, u) => {
    ...
    // Auto-fill the candidate's photo from their member profile, if they
    // have one uploaded — the admin no longer has to paste a URL by hand.
    const photo = getProfilePhoto(u);
    if (photo) setCandField(pi, ci, 'photo', photo);
  ...
  const inputCls = "w-full px-4 py-3.5 border border-gray-300 rounded-xl text-base focus:outline-none focus:border-[#E30613] focus:ring-2 focus:ring-[#E30613]/30 transition";

  return (
    <div className="min-h-screen bg-gray-50 py-6 px-3 sm:py-12 sm:px-4">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-4 sm:p-10">
        <h1 className="text-2xl sm:text-4xl font-bold text-[#001F5B] mb-2 text-center">Create Election</h1>
        ...
        {feedback && (
          <div className={`mb-5 p-4 rounded-xl font-medium text-center flex items-center justify-center gap-2 text-sm ${
        ...
        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                    ...
                    {pos.candidates.map((c, ci) => {
                      ...
                      return (
                        <div key={ci} className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 relative">
                          ...
                          {c.userId && (
                            <p className="text-xs text-green-600 font-medium ml-1 mb-2 flex items-center gap-1">
          ...
          <button type="submit" disabled={loading}
            className={`w-full py-4 sm:py-5 rounded-xl text-white font-bold text-lg sm:text-xl transition flex items-center justify-center gap-3 ${
