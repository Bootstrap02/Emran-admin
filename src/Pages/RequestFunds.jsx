// pages/admin/RequestFunds.jsx
// Admin submits a payment request — form auto-fills requester details from localStorage
...
const RequestFunds = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!admin?._id) { setFeedback({ type: 'error', text: 'Admin session not found. Please log in again.' }); return; }
    setLoading(true);
    try {
      await axios.post(`${API}/request/${admin._id}`, form);
      setFeedback({ type: 'success', text: 'Payment request submitted successfully. The President has been notified.' });
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-8 space-y-6">
        {/* Purpose */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1.5">Purpose / Reason for Request <span className="text-red-500">*</span></label>
        ...
        {/* Amount */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1.5">Amount Requested (₦) <span className="text-red-500">*</span></label>
        ...
        <div className="border-t pt-5">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Beneficiary Name <span className="text-red-500">*</span></label>
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? <><FiLoader className="animate-spin" /> Submitting...</> : <><FiDollarSign /> Submit Payment Request</>}
        </button>
      </form>
    </div>
  );
