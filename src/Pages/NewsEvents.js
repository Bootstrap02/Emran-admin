...
const NewsEvents = () => {
  ...
  const fetchItems = async () => {
    const res = await axios.get('/api/admin/news-events');
    setItems(res.data);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (form.id) {
      await axios.put(`/api/admin/news-events/${form.id}`, form);
    } else {
      await axios.post('/api/admin/news-events', form);
    }
    fetchItems();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete item?')) {
      await axios.delete(`/api/admin/news-events/${id}`);
      fetchItems();
    }
  };
  ...
  return (
    <div>
      <h1 className="text-4xl font-bold text-[#001F5B] mb-8">Manage News & Events</h1>

      {/* Form */}
      <div className="bg-white rounded-3xl shadow-xl p-8 mb-12">
        <h2 className="text-2xl font-bold text-[#001F5B] mb-6">Add/Edit Item</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <textarea
            ...
            className="w-full p-6 border-2 border-gray-200 rounded-2xl text-lg focus:border-[#E30613] h-48"
            required
          />
        ...
        {items.map(item => (
          <div key={item.id} className="bg-white rounded-3xl shadow-xl p-8 flex justify-between items-start">
            <div>
              ...
              <p className="text-lg text-gray-700">{item.content.substring(0, 200)}...</p>
            </div>
