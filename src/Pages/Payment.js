...
//     const fetchConfirmedPayments = async () => {
//       try {
//         // Parse inside effect for stability
//         const admin = JSON.parse(localStorage.getItem('admin')) || { paymentApprovals: [] };
//         const dummy = admin.paymentApprovals;
//         setPayments(dummy);
//         setLoading(false);
//       } catch (err) {
...
export const ConfirmedPayments = () => {
  ...
  const handleConfirm = async (payment) => {
    ...
    try {
      ...
      const targetUrl = payment.year && payment.year === currentYear + 1
        ? approveNextPayment
        : approveCurrentPayment;
                ...
                {payments.length === 0 ? (
                  ...
                  payments.map(payment => (
                    <tr key={payment.userId} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-6 whitespace-nowrap text-gray-700 font-medium">{payment.paymentToken}</td>
                      <td className="px-6 py-6 whitespace-nowrap text-gray-600">
